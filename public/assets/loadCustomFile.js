...
for (let i = 0; i < arrScript.length; i++) {
    loadScript(`${base_url}/portfolio/pizzawebapp/public/assets/${arrScript[i]}`).then(data => { }).catch(err => { console.error(err) })
}

// Loading link dynamically
for (let i = 0; i < arrLink.length; i++) {
    (i == 0) ? type = 'image/png' : type = 'stylesheet'
    loadLink(`${base_url}/portfolio/pizzawebapp/public/assets/${arrLink[i]}`, type).then(data => { }).catch(err => { console.error(err) })
}